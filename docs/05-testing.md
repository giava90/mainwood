...
```bash
python -m pytest          # from the repository root, ~5 s, 84 tests
```
...
| `test_summary_end_to_end.py` | the whole read chain on a real SorSim file | Header-block detection and the Windows-1252 umlauts |
